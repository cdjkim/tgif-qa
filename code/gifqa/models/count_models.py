...
class CountOF(CountBase):
    @staticmethod
    def add_flags(FLAGS):
        FLAGS.image_feature_net = "optflow"
        FLAGS.layer = "pool5"
class CountOFEvaluator(CountBaseEvaluator):
    pass
class CountOFTrainer(CountBaseTrainer):
    pass
...
class CountTp(CountBase):
    ...
    def build_graph(self,
        ...
        with tf.variable_scope("conv_image_emb"):
            self.r_shape = tf.reshape(
                self.video, [-1, self.kernel_size, self.kernel_size, self.channel_size])
            #  [batch_size*length, kernel_size, kernel_size, channel_size]
            self.pooled_feat = tf.nn.avg_pool(
                self.r_shape,
    ...
    def build_graph_single_gpu(self, video, video_mask, caption, caption_mask, answer, idx):
        with tf.variable_scope("word_emb"):
            with tf.device("/cpu:0"):
                ...
                # [batch_size, length, word_dim]
                embedded_start_word = tf.nn.embedding_lookup(self.word_embed_t,
                                                                  tf.ones([self.batch_size_per_gpu], dtype=tf.int32))
